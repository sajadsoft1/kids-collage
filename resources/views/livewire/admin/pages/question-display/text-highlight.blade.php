<div class="space-y-4" x-data="{
    ...
    @if ($question->body)
        <div id="th-body"
            class="p-4 max-w-none text-gray-700 bg-gray-50 rounded-lg whitespace-pre-wrap cursor-text select-text">
            {!! e($question->body) !!}</div>
    @endif
    ...
    <div class="flex items-center justify-between gap-2 text-sm">
        <div class="flex items-center gap-2">
            <x-badge :value="count($value['selections']) . ' انتخاب'" :class="count($value['selections']) >= $this->getMinSelections() ? 'badge-success' : 'badge-warning'" />
            ...
            <div class="space-y-1">
                @foreach ($value['selections'] as $index => $selection)
                    ...
                    <div class="flex items-center justify-between p-2 bg-gray-50 rounded"
                        wire:key="sel-display-{{ $index }}">
                        <span class="text-sm text-gray-700">{{ $selectedText }}</span>
                        <x-button size="xs" icon="o-x-mark" wire:click="removeSelection({{ $index }})"
