                        ...
                        <li class="flex justify-between items-center py-3 border-b border-slate-200">
                            ...
                            <div class="ml-2 text-sm font-medium text-slate-800">
                                <x-input :label="trans('validation.attributes.discount')" :placeholder="trans('validation.attributes.discount')" wire:model="discount_code"
                                    inputmode="numeric" pattern="[0-9]*" inline required :disabled="$edit_mode || !$this->payableAmount">
                                    <x-slot:append>
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
                            <x-select :label="trans('datatable.payment_method')" :placeholder="trans('datatable.payment_method')" placeholder-value="0" :options="PaymentTypeEnum::options()"
                                option-label="label" option-value="value" :disabled="$payment['status'] === PaymentStatusEnum::PAID->value"
                                wire:model.live="payments.{{ $index }}.payment_type" inline required />

                            <x-input :label="trans('validation.attributes.price')" :placeholder="trans('validation.attributes.price')" :disabled="$payment['status'] === PaymentStatusEnum::PAID->value"
                                wire:model="payments.{{ $index }}.amount" inputmode="numeric" pattern="[0-9]*"
                                inline required />

                            <x-admin.shared.smart-datetime label="" :placeholder="trans('validation.attributes.date')" :disabled="$payment['status'] === PaymentStatusEnum::PAID->value"
                                wire:model="payments.{{ $index }}.scheduled_date" inline setNullInput="true"
                                required />
