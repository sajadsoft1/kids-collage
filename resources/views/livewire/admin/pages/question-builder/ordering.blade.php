<div class="space-y-4">
    <div class="space-y-2">
        @foreach ($options as $index => $item)
            <div wire:key="item-{{ $index }}" class="flex items-center gap-2 p-3 rounded bg-gray-50">
                <div class="flex items-center gap-1">
                    <x-button size="xs" icon="o-chevron-up" wire:click="moveUp({{ $index }})"
                        class="btn-ghost" />
                    <x-button size="xs" icon="o-chevron-down" wire:click="moveDown({{ $index }})"
                        class="btn-ghost" />
                </div>
                <div class="w-8 text-xs text-center">{{ $index + 1 }}</div>
    ...
    <x-button icon="o-plus" wire:click="addItem" class="w-full btn-outline">افزودن مورد</x-button>
    ...
    <div x-data="{ draggedIndex: null }">
        ...
        <div class="mb-4 space-y-3">
            @foreach ($options as $index => $option)
                <div wire:key="option-{{ $index }}" draggable="true"
                    x-on:dragstart="draggedIndex = {{ $index }}" x-on:dragover.prevent
                    x-on:drop.prevent="
        ...
        <div class="p-3 mt-4 rounded-lg bg-blue-50">
            <p class="text-sm text-blue-800">
                <x-heroicon-o-information-circle class="inline w-4 h-4 ml-1" />
