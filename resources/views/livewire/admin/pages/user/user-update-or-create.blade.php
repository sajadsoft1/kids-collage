            ...
            <div class="sticky top-20">
                ...
                <x-card :title="trans('user.page.parents_info')" shadow separator progress-indicator="submit" class="">
                    ...
                    @elseif($this->detected_user_type==UserTypeEnum::USER)
                        <x-select :label="trans('validation.attributes.father_name')"
                                  wire:model="father_id" :options="$male_parents"
                                  option-value="value"
                                  :placeholder="trans('user.page.select_father_placeholder')"
                                  placeholder-value="0"
                                  option-label="label"/>
                        <x-select :label="trans('validation.attributes.mother_name')" wire:model="mother_id" :options="$female_parents" option-value="value"
                                  :placeholder="trans('user.page.select_mother_placeholder')"
                                  placeholder-value="0"
                                  option-label="label"/>
                    @else
                        <x-choices :label="trans('validation.attributes.children')" wire:model="children_id" :options="$childrens" option-value="value"
                                   :placeholder="trans('user.page.select_mother_placeholder')"
                                   placeholder-value="0"
                                   option-label="label"/>
                    @endif
                ...
                @if(in_array($this->detected_user_type,[UserTypeEnum::EMPLOYEE,UserTypeEnum::TEACHER]))
                    <x-card :title="trans('user.page.salary_info')" shadow separator progress-indicator="submit" class="mt-5">
                        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                            <x-input :label="trans('validation.attributes.salary')" wire:model="salary" min="0" icon="lucide.hand-coins" :suffix="systemCurrency()"/>
                            <x-input :label="trans('validation.attributes.benefit')" wire:model="benefit" min="0" icon="lucide.hand-coins" :suffix="systemCurrency()"/>
                            <x-admin.shared.smart-datetime :label="trans('validation.attributes.cooperation_start_date')" wire-property-name="cooperation_start_date"/>
                            <x-admin.shared.smart-datetime :label="trans('validation.attributes.cooperation_end_date')" wire-property-name="cooperation_end_date"/>
                        </div>
                    </x-card>
                @endif
