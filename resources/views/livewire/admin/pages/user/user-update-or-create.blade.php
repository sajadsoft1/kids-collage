    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div class="grid grid-cols-1 col-span-2 gap-4">
            <x-card :title="trans('general.page_sections.data')" shadow separator progress-indicator="submit">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <x-admin.shared.smart-datetime :label="trans('validation.attributes.birth_date')" wire:model="birth_date" />

                    <x-input :label="trans('validation.attributes.national_code')" wire:model="national_code" type="number" />
                </div>
            </x-card>

            <x-card :title="trans('user.page.images_section')" shadow separator progress-indicator="submit">
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                    <x-admin.shared.single-file-upload wire_model="avatar" :default_image="$user->getFirstMediaUrl('avatar', Constants::RESOLUTION_480_SQUARE)" :crop_after_change="true" />
                    <x-admin.shared.single-file-upload wire_model="national_card" :default_image="$user->getFirstMediaUrl('national_card', Constants::RESOLUTION_480_SQUARE)"
                        :crop_after_change="true" />
                    <x-admin.shared.single-file-upload wire_model="birth_certificate" :default_image="$user->getFirstMediaUrl('birth_certificate', Constants::RESOLUTION_480_SQUARE)"
                        :crop_after_change="true" />
                </div>
            </x-card>
        </div>
        <div class="col-span-1 ">
            <div class="sticky top-20">
                <x-card :title="trans('user.page.parents_info')" shadow separator progress-indicator="submit" class="mt-5">
                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
