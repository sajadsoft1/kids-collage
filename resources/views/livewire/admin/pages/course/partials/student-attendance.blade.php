...
<x-card>
    ...
    @if ($attendance)
        <div class="space-y-3">
            {{-- Status --}}
            <div
                class="flex items-center justify-between p-3 rounded-lg {{ $attendance->present->value ? 'bg-success/10 border border-success/20' : 'bg-error/10 border border-error/20' }}">
            ...
            {{-- Details - Compact --}}
            @if ($attendance->present->value)
                <div class="p-3 rounded-lg border border-base-300 space-y-2">
                    ...
                    @if ($attendance->isLate())
                        <x-alert icon="o-exclamation-triangle" class="alert-warning text-xs py-2">
                            {{ __('attendance.page.delay') }}: {{ $attendance->lateness_minutes }}
                            {{ __('attendance.page.minutes') }}
                        </x-alert>
                    @endif
                </div>
            @else
                @if ($attendance->excuse_note)
                    <div class="p-3 rounded-lg border border-base-300">
                        <div class="text-xs text-base-content/60 mb-1">{{ __('attendance.page.excuse') }}</div>
