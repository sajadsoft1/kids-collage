            ...
            <x-card :title="trans('general.page_sections.data')" shadow separator progress-indicator="submit">
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <x-admin.shared.smart-datetime :label="trans('validation.attributes.expired_at')"
                                                   :default-date="$expired_at"
                                                   wire-property-name="expired_at"/>


                </div>
            </x-card>

            <x-card :title="trans('general.roles')" shadow separator progress-indicator="submit"
                    class="mt-5 relative min-h-[400px]">
                <x-slot:menu>
        ...
        <div class="col-span-1">
            <div class="sticky top-20">
                <x-card :title="trans('general.page_sections.upload_image')" shadow separator
                    ...
                    <x-admin.shared.single-file-upload wire_model="image"
                ...
                <x-card :title="trans('setting.model')" shadow separator progress-indicator="submit" class="mt-5">
                    <div class="grid gap-4 grid-col-1">
                        {{--                        <x-toggle :label="trans('validation.attributes.has_timer')" wire:model.live="has_timer" right--}}
                        {{--                                  value="1"/>--}}
                        {{--                        <div wire:show="has_timer">--}}
                        {{--                            <x-admin.shared.smart-datetime :label="trans('validation.attributes.timer_start')"--}}
                        {{--                                                           :default-date="$timer_start"--}}
                        {{--                                                           wire-property-name="timer_start"/>--}}
                        {{--                        </div>--}}
                        <x-input :label="trans('validation.attributes.ordering')" wire:model="ordering" type="number"
                                 required/>
                    </div>
                </x-card>
            </div>
