<div class="py-5">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <x-stat title="{{ __('widgets.teacher_classes.stats.total') }}" value="{{ $this->classStats['total'] }}"
            ...
            tooltip="{{ __('widgets.teacher_classes.stats.total_tooltip') }}" />
        ...
        @if ($this->classes->count() > 0)
            <div class="space-y-3">
                @foreach ($this->classes as $index => $class)
                    <x-list-item :item="$class" no-separator wire:key="teacher-classes-widget-{{ $class->id }}-{{ $index }}">
                        <x-slot:avatar>
                        ...
                        <x-slot:value>
                            <div class="font-medium">{{ $class->template->title ?? __('widgets.teacher_classes.unknown_course') }}</div>
