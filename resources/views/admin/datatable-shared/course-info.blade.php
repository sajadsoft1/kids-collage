...
<x-avatar :image="$row->template->getFirstMediaUrl('image', Constants::RESOLUTION_100_SQUARE)" class="!w-14 !rounded-lg">
    <x-slot:title class="!font-bold ps-1">
        <a href="{{ route('admin.course.show', ['course' => $row->id, 'courseTemplate' => $row->template->id]) }}"
            target="_blank">{{ $row->template->title }}</a>
    </x-slot:title>

    <x-slot:subtitle class="grid gap-1 mt-1 text-xs ps-1">
