    ...
    <form wire:submit="saveModal" id="modal-form">
            ...
            @foreach ($this->modalFields() as $field)
                ...
                @elseif ($field['type'] === 'select')
                    <x-select :label="$label" wire:model="{{ $wireModel }}" :options="$field['options'] ?? []" :placeholder="$placeholder"
                        :required="$required" :icon="$field['icon'] ?? null" wire:loading.attr="disabled" wire:target="saveModal" />
                ...
                @elseif ($field['type'] === 'email')
                    <x-input type="email" :label="$label" wire:model="{{ $wireModel }}" :placeholder="$placeholder"
                        :required="$required" icon="o-envelope" wire:loading.attr="disabled" wire:target="saveModal" />
        ...
        <x-slot:actions separator>
            <div class="flex gap-3 justify-end">
                ...
                {{-- Submit Button --}}
                <x-button type="submit" class="btn-primary" :label="$this->editingId ? trans('general.edit') : trans('general.create')" icon="o-check" spinner="saveModal"
                    form="modal-form" />
            </div>
        </x-slot:actions>
    </form>
