...
<div id="sidebar"
    class="fixed top-0 bottom-0 z-20 items-stretch shrink-0 w-[290px] bg-[#F6F6F9] dark:bg-[#1e1e2d] hidden lg:flex"
    :class="{
        'flex': sidebarOpen || window.innerWidth >= 1024,
        'hidden': !sidebarOpen && window.innerWidth < 1024
    }"
    ...
    <div class="flex flex-col items-stretch shrink-0 gap-5 py-5 w-[70px] border-e border-gray-300 dark:border-gray-700"
        id="sidebar_primary">
        ...
        <div class="hidden lg:flex items-center justify-center shrink-0" id="sidebar_primary_header">
            <a href="{{ route('admin.dashboard') }}">
                ...
                <img class="hidden dark:block min-h-[30px]"
                    src="{{ asset('assets/media/app/mini-logo-gray-dark.svg') }}" alt="Logo">
            </a>
        </div>
        ...
        <div class="flex flex-col gap-5 items-center shrink-0" id="sidebar_primary_footer">
            <div class="flex flex-col gap-1.5">
                ...
                <button @click="toggleTheme()"
                    ...
                    <svg x-show="themeMode === 'dark'" class="w-5 h-5" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="menu">
                    <button class="btn btn-icon rounded-full">
                        <img src="{{ auth()->user()->avatar_url ?? 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) }}"
                            alt="{{ auth()->user()->name }}" class="size-8 rounded-full border border-gray-500 shrink-0">
                    </button>
                </div>
            @endauth
        </div>
            ...
            <div class="menu flex flex-col w-full gap-px px-2.5">
                ...
                <div class="menu-item {{ request()->routeIs('admin.metronic.test') ? 'active' : '' }}">
                    <a href="{{ route('admin.dashboard') }}"
                        ...
                        <span
                            class="menu-title text-2sm text-gray-800 {{ request()->routeIs('admin.metronic.test') ? 'font-medium text-primary' : '' }}">
                            Get Started
                        </span>
                    </a>
                </div>
                ...
                <div class="menu-item" x-data="{ open: false }">
                    ...
                    <div x-show="open" x-collapse class="menu-accordion gap-px">
                        <div class="menu-item">
                            <a href="#"
                                class="menu-link py-2 ps-5 pe-2.5 rounded-md border border-transparent hover:bg-light hover:border-gray-200">
...
{{-- Mobile Backdrop --}}
<div x-show="sidebarOpen" @click="toggleSidebar()" x-transition:enter="transition-opacity ease-linear duration-300"
    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
    x-transition:leave="transition-opacity ease-linear duration-300" x-transition:leave-start="opacity-100"
    x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black bg-opacity-50 z-10 lg:hidden"
    style="display: none;">
</div>
