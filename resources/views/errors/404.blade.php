@php
    ...
    $description = __('error.404.description');
...
<x-layouts.error>
    ...
    <div class="mt-[16px] flex flex-col md:flex-row justify-center items-center gap-3">
        @if ($isAuthenticated)
            <a href="{{ route('admin.dashboard') }}" class="w-full error-btn md:w-auto">
                {{ __('error.back_to_dashboard') }}
            </a>
        @endif
        <button onclick="window.history.back()" class="w-full error-btn md:w-auto">
            {{ __('error.go_back') }}
        </button>
    </div>
</x-layouts.error>
