        ...
        <script>
            window.addEventListener('load', function() {
                @foreach ($statuses as $status)
                    ...
                    const status{{ $status['id'] }}Element = document.getElementById(
                        '{{ $status['statusRecordsId'] }}');

                    Sortable.create(status{{ $status['id'] }}Element, {
                        ...
                        onEnd: function(evt) {
                            ...
                            $wire.call('onStatusChanged', recordId, toStatusId, fromOrderedIds,
                                toOrderedIds);
                        },
                    });
                @endforeach
